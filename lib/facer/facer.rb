class Facer
  def self.face_detector
    @@face_detector ||= OpenCV::CvHaarClassifierCascade::load(File.dirname(__FILE__) + "/../../fixtures/haarcascade_frontalface_alt.xml")
  end
  ...
  def self.put_funny_hat_on_celebrity(celebrity_img)
    ...
    face_rectangles.each do |rect|
      ...
      hat = hat.resize(rect_w.to_f * 0.8, rect_h.to_f * 0.8)
      
      x_offset = rect.top_left.x - (hat.columns.to_f * 0.1)
      y_offset = rect.top_left.y - (hat.rows.to_f * 0.95)
      r_img.composite!(hat, x_offset, y_offset, Magick::OverCompositeOp)
    end
    r_img
  end
